...
describe("RoleplayX projection adapter", () => {
  it("maps canonical graph to scenario JSON", () => {
    const graph = validContentGraph();
    const scenario = projectToRoleplayX(graph);
  ...
  it("maps characters to personas with roles and traits", () => {
    const scenario = projectToRoleplayX(validContentGraph());
  ...
  it("records projection meta and does not mutate the canonical graph", () => {
    const graph = validContentGraph();
    const before = JSON.stringify(graph);
    const scenario = projectToRoleplayX(graph);
    expect(scenario.meta.sourceContentId).toBe("content_demo1");
    expect(scenario.meta.sourceVersion).toBe(3);
    expect(scenario.meta.adapter).toBe("roleplayx@1");
