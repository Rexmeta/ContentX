  ...
  const contextHeader = (
    <div className="flex items-center justify-between">
      <div className="space-y-1">
        <h1 className="headline-lg flex items-center gap-2" title={agent.name}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
          ...
          <div className="md:col-span-2 space-y-8">
            ...
            <section className="space-y-4">
              <h2 className="tech-label text-base border-b border-border pb-2">Goals & Constraints</h2>
              <div className="space-y-4">
                {agent.goals?.map((goal, i) => (
                  <div key={i} className="border border-border rounded-xl p-4 bg-card">
                    ...
                    <div className="tech-label text-muted-foreground mt-2">PRIORITY: {goal.priority}</div>
                  </div>
                ))}
            ...
            <section className="space-y-4">
              <h2 className="tech-label text-base border-b border-border pb-2">AgentState (Mutable Runtime State)</h2>
              {agent.state ? (
                ...
                <div className="text-sm text-muted-foreground italic border border-border rounded-xl p-4 bg-card text-center">
            ...
            <section className="space-y-4">
              <h2 className="tech-label text-base border-b border-border pb-2">Memory</h2>
          ...
          <div className="space-y-6">
            <div className="border border-border rounded-xl bg-card p-4">
              <h3 className="tech-label text-muted-foreground mb-4">Provenance</h3>
              <div className="space-y-4">
                <div>
            ...
            <div className="border border-border rounded-xl bg-card p-4">
              <h3 className="tech-label text-muted-foreground mb-4">Configuration</h3>
              <div className="space-y-4">
                <div>
                  <div className="text-xs text-muted-foreground mb-1">Policy</div>
                  <pre className="text-[10px] font-mono bg-muted/20 p-2 rounded-lg border border-border overflow-auto max-h-40">
            ...
            <div className="border border-border rounded-xl bg-card p-4">
              <h3 className="tech-label text-muted-foreground mb-4">Associated Simulations ({simulations.length})</h3>
              <div className="space-y-2">
                {simulations.length > 0 ? (
                  simulations.map(sim => (
                    <Link key={sim.id} href={`/simulations/${sim.id}`} className="block p-2 border border-border rounded-lg bg-muted/20 hover:border-primary transition-colors">
                      <div className="text-sm font-bold truncate" title={sim.name}>{formatDisplayName(sim.name)}</div>
                      <div className="tech-label text-muted-foreground mt-1">Turns: {sim.turnsExecuted}</div>
                    </Link>
                  ))
                ) : (
