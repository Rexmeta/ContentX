...
- Dashboard: telemetry, content library, scenario library, and a two-step creation workflow — idea → real-LLM-amplified dramatic scenario draft (logline, synopsis, theme/stakes/twist, 3 acts, characters; user-editable; takes ~20-40s) → explicit user confirm → Content Graph commit (`POST /v1/scenarios/draft` drafts without persisting; `POST /v1/content` with `scenario` commits; provenance operation `compose`)
- Scenario amplification uses Replit AI Integrations (OpenAI, `gpt-5.6-terra`) via `@workspace/integrations-openai-ai-server`; LLM output is Zod-validated with no silent fallback (`AmplificationError` → 502). Deterministic mock amplifier remains for tests (`mockAmplifier`).
- Scenario library: scenarios persist in the `scenarios` table (JSONB) with CRUD at `/v1/scenarios`; saved scenarios can be edited, re-saved, and committed to graphs
- Category clustering: scenarios are auto-classified on save/update (domain/conflictType/tone/tags) via LLM; seed taxonomy in `domains/scenario/taxonomy.ts` auto-extends with classifier-proposed categories (`categories` table, origin seed/auto). Endpoints: `GET /v1/categories`, `POST /v1/scenarios/{id}/classify`, `POST /v1/scenarios/reclassify`, `GET /v1/scenarios/{id}/similar` (shared domain or conflictType). Classification failure never blocks a save — record stays explicitly unclassified (`classification: null`); manual override via `PATCH /v1/scenarios/{id}` with `classification`
- Scenario synthesis (remix): `POST /v1/scenarios/synthesize` combines selected elements (characters/conflict/setting/twist/structure) from 2+ saved scenarios via LLM (`domains/scenario/synthesizer.ts`, `SynthesisError` → 502). Lineage (parents + borrowed elements + instruction + synthesizedBy) is server-authoritative — validated/rebuilt by `domains/scenario/lineageService.ts` on scenario save (`scenarios.lineage` JSONB) and on graph creation (`POST /v1/content` with `lineage` → stored in `provenance.lineage`), so SYNTHESIZED provenance cannot be forged. Synthesized scenarios are classified/filterable/re-synthesizable like any other
